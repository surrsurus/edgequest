...
from settings.settings import COLOR_JSON_PATH, CURRENT_THEME
import random
...
def hex_to_color(inp):
    ...
    try:
        ...
        logger.debug('Color: (' + str(red_bit) + ', ' + str(green_bit) + ', ' + \
            str(blue_bit) + ')')
...
def get_color(col):
    ...
    if type(col) is unicode or type(col) is str:
        return hex_to_color(col)
    elif type(col) is list:
        return libtcod.Color(*col)
    else:
        logger.error('Invalid argument passed to get_color(). Type is: `' + str(type(col)) + '`')
...
def set_theme(theme):
    ...
    if theme is not None:
        ...
        try:
            ...
            logger.error('KeyError: JSON error occurred while loading... Using defaults...')
            default_colors()
        ...
        try:
            ...
            logger.error('AssertionError: Looks like there is invalid or missing data in the `' +\
